...
class BaseAviary(BenchmarkEnv):
    ...
    def __init__(self,
        ...
        super().__init__(gui=gui, verbose=verbose, obs_goal_append_positions_only=obs_goal_append_positions_only, **kwargs)
        # Connect to PyBullet.
        self.PYB_CLIENT = -1
        if gui:
            # With debug GUI.
    ...
    def setup_dynamics_si_3d_delay_expression(self, prop_values=None):
        ...
        if model_choice == "drag":
            ...
            # Dynamics parameters
            params_acc =  prop_values.get('params_acc', [0.13, 0.72, 0.08, -0.0171, 0.000811])
            params_roll_rate = prop_values.get('params_roll_rate', [-238.1, -21.35, 179.65])
            params_pitch_rate = prop_values.get('params_pitch_rate', [-238.1, -21.35, 179.65])
            params_yaw_rate = prop_values.get('params_yaw_rate', [-170.4, -22.22, 280])
            # print("Using dynamics params_acc:", params_acc)
            # print("Using dynamics params_roll_rate:", params_roll_rate)
            # print("Using dynamics params_pitch_rate:", params_pitch_rate)
            # print("Using dynamics params_yaw_rate:", params_yaw_rate)
