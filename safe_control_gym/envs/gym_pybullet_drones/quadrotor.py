...
class Quadrotor(BaseAviary):
    ...
    def set_goals(self):
        ...
        if self.TASK == Task.STABILIZATION:
            ...
            if use_ilqr_ref:
                ...
                print('Using iLQR reference trajectory for quadrotor.')
                if hasattr(self.TASK_INFO, 'ilqr_traj_data'):
                    traj_data = np.load(self.TASK_INFO['ilqr_traj_data'], allow_pickle=True).item()
                    traj_data = np.load('/home/benchmark/safe-control-gym/benchmarking_sim/quadrotor/data/mpc_acados_quadrotor_3D_attitude_obstacle_ref_traj.npy', allow_pickle=True).item()
                else:
    ...
    def _setup_symbolic(self, prior_prop={}, **kwargs):
        ...
        if self.QUAD_TYPE == QuadType.ONE_D:
            ...
            # params_acc = prior_prop.get('params_acc', [0.182, 0.650, 0.120])  # from the identified model
