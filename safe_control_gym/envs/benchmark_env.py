    ...
    def _generate_trajectory(self,
        ...
        if traj_type == 'snap_figure8':
            ...
            if string_list is not None:
                ...
                # Plot waypoints generated by the MPC before applying minimum snapAdd commentMore actions
                pos_ref = np.array([wp.position for wp in waypoints])
                script_dir = os.path.dirname(__file__)
                _plot_trajectory(pos_ref, waypoints=waypoints, strings=string_list, save_path=os.path.join(script_dir, '../../benchmarking_sim/quadrotor/data', 'MPC_trajectory.png'))
                _plot_xyz_kinematics(pos_ref, None, None, None, waypoints=waypoints, strings=string_list, save_path=os.path.join(script_dir, '../../benchmarking_sim/quadrotor/data', 'MPC_trajectory.png'))
            else:
