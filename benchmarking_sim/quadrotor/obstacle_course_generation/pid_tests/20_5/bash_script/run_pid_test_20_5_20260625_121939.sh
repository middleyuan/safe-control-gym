...
DATA_DIR="$REPO_ROOT/benchmarking_sim/quadrotor/obstacle_course_generation/data/20_5"
NPY_PATH="$DATA_DIR/trajectory/custom_snap_ref_traj_20_5.npy"
EP_LEN="20.5"

STAMP="$(date +%Y%m%d_%H%M%S)"
PID_TEST_DIR="$REPO_ROOT/benchmarking_sim/quadrotor/obstacle_course_generation/pid_tests/20_5"
...
cp "$0" "$BASH_SCRIPT_DIR/run_pid_test_20_5_$STAMP.sh"

cd "$REPO_ROOT/examples/pid"

# PID Experiment for 20.5s obstacle reference.
python3 ./pid_experiment.py \
    ...
    2>&1 | tee "$BASH_SCRIPT_DIR/run_pid_test_20_5_$STAMP.log"
