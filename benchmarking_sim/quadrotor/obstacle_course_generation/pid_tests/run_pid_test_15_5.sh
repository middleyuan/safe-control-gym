...
DATA_DIR="$REPO_ROOT/benchmarking_sim/quadrotor/obstacle_course_generation/data/15_5"
NPY_PATH="$DATA_DIR/trajectory/custom_snap_ref_traj_15_5.npy"
EP_LEN="15.5"

STAMP="$(date +%Y%m%d_%H%M%S)"
PID_TEST_DIR="$REPO_ROOT/benchmarking_sim/quadrotor/obstacle_course_generation/pid_tests/15_5"
...
cp "$0" "$BASH_SCRIPT_DIR/run_pid_test_15_5_$STAMP.sh"

cd "$REPO_ROOT/examples/pid"

# PID Experiment for 15.5s obstacle reference.
python3 ./pid_experiment.py \
    ...
    2>&1 | tee "$BASH_SCRIPT_DIR/run_pid_test_15_5_$STAMP.log"
